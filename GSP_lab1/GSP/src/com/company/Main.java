...
public class Main extends Application {
    ...
    public void start(Stage stage) throws Exception {
        ...
        Scene scene = new Scene(root, 1350, 1080);
        ...
        int n = scan.nextInt();
        scan.nextLine();
        System.out.println("Граф орієнтовний?");
        String s = scan.nextLine();
        boolean orientation = s.equals("Так");
        ...
        MyNode[] nodes = makeNodes(n);
        for (MyNode node : nodes) {
            root.getChildren().add(node.circle);
            root.getChildren().add(node.textOfNode);
        }

        ArrayList<Node> edges = makeEdge(n, a, nodes, orientation);
        for (Node edge : edges) {
    ...
    public double[] equalationOfLine(double x1, double y1, double x2, double y2) {
        return new double[]{(y2-y1)/(x2-x1), (y1*x2 - x1*y2)/(x2-x1)};
    }

    public double[] lineCircleIntersection(double k, double b, double otX, double otY, double kuX, double kuY, double r) {
    ...
    public double[] findControlXY(MyNode ot, MyNode ku) {
        ...
        double midX = (x01+x02)/2;
        double midY = (y01+y02)/2;
    ...
    public double[] findControlXYoriented(MyNode ot, MyNode ku) {
        double x01 = ot.circle.getCenterX();
        double y01 = ot.circle.getCenterY();
        double x02 = ku.circle.getCenterX();
        double y02 = ku.circle.getCenterY();
    ...
    public double lenOfLine(double x1, double y1, double x2, double y2) {
        return Math.sqrt(Math.pow((x1-x2), 2)+Math.pow((y1-y2), 2));
    }

    public Node makeNoLoopEdge(int n, MyNode ot, MyNode ku, boolean flag, MyNode[] nodes) {
        ...
        if ((ot.isHaveDoubleEdge.equals("No")) && (numOfNbrL == ku.numberOfNode || numOfNbrR == ku.numberOfNode)
            ...
            double[] equalOfLine = equalationOfLine(ot.circle.getCenterX(), ot.circle.getCenterY(), ku.circle.getCenterX(), ku.circle.getCenterY());
            ...
            if (flag) {
                return new ArrowLine(otX, otY, kuX, kuY);
            }
            else {
            ...
            double[] eqaulOflineKu = equalationOfLine(ku.circle.getCenterX(), ku.circle.getCenterY(), controlX, controlY);
            kKu = eqaulOflineKu[0];
            bKu = eqaulOflineKu[1];
    ...
    public Node makeLoopEdge(MyNode node) {
        ...
        loopEdge.setStroke(Color.BLACK);
        loopEdge.setStrokeWidth(2);
        return loopEdge;
    }

    public ArrayList<Node> makeEdge(int n, int[][] matrix1, MyNode[] nodes, boolean orientation) {
        ArrayList<Node> edgeList = new ArrayList<>();
        int[][] a = Arrays.copyOf(matrix1, n);
        for (int i = 0; i < a.length; i++){
            for (int j = 0; j < a[i].length; j++){
                if (a[i][j] == 1 && a[j][i] == 1 && i!=j && orientation) {
                    nodes[i].isHaveDoubleEdge = "Creater";
                    nodes[j].isHaveDoubleEdge = "Inheriter";
                    edgeList.add(makeNoLoopEdge(n, nodes[i], nodes[j], orientation, nodes));
                    edgeList.add(makeNoLoopEdge(n, nodes[j], nodes[i], orientation, nodes));
                    nodes[i].isHaveDoubleEdge = "No";
                    nodes[j].isHaveDoubleEdge = "No";
                    a[i][j] = -1;
                    a[j][i] = -1;
                } else if (a[i][j] > 0 &&  i == j) {
                    edgeList.add(makeLoopEdge(nodes[i]));
                } else {
                    if (a[i][j] == 1){
                        edgeList.add(makeNoLoopEdge(n, nodes[i], nodes[j], orientation, nodes));
                    }
    ...
    public MyNode[] makeNodes(int n) {
        MyNode[] nodes = new MyNode[n];
        if (n == 1) {
            nodes[0] = new MyNode(660, 500, 25, 1, 1);
        } else if (n == 2) {
            nodes[0] = new MyNode(660, 350, 25, 1, 1);
            nodes[1] = new MyNode(660, 650, 25, 2, 2);
        } else {
